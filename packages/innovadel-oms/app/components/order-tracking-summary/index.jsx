...
const OrderSummary = ({
    ...
    if (!basket?.OrderSummary?.OrderNumber && !basket?.c_orderData?.OrderSummary.OrderNumber) {
        return null
    }
    const shippingItem = basket.shippingItems?.[0]
    const hasShippingPromos = shippingItem?.priceAdjustments?.length > 0
    const orderPriceAdjustments = basket?.orderPriceAdjustments || basket?.c_orderData?.orderPriceAdjustments
    const orderTaxAmount = basket?.OrderSummary?.TotalTaxAmount || basket?.c_orderData?.OrderSummary.TotalTaxAmount
                ...
                <Stack w="full">
                    <Flex justify="space-between">
                        ...
                        <Text fontWeight="bold" fontSize={fontSize}>
                            <FormattedNumber
                                style="currency"
                                currency={basket?.currency || basket?.c_orderData?.currency}
                                value={basket?.OrderSummary?.TotalAdjustedProductAmount || basket?.c_orderData?.OrderSummary.TotalAdjustedProductAmount}
                            />
                        </Text>
                    </Flex>

                    {orderPriceAdjustments?.map((adjustment) => (
                    ...
                    <Flex justify="space-between">
                        ...
                        {shippingItem?.priceAdjustments?.some(
                            ...
                            <Text fontSize={fontSize}>
                                <FormattedNumber
                                    value={basket?.OrderSummary?.TotalDeliveryAmount || basket?.c_orderData?.OrderSummary.TotalDeliveryAmount}
                                    style="currency"
                                    currency={basket?.currency || basket?.c_orderData?.currency}
                                />
                            </Text>
                    ...
                    <Flex w="full" justify="space-between">
                        ...
                        <Text fontWeight="bold" fontSize={fontSize}>
                            <FormattedNumber
                                style="currency"
                                currency={basket?.currency || basket?.c_orderData?.currency}
                                value={basket?.OrderSummary?.GrandTotalAmount || basket?.c_orderData?.OrderSummary?.GrandTotalAmount}
                            />
                        </Text>
                    </Flex>
