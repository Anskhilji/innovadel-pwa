...
import { RatingStar } from '../product-rating'
import TagManager from 'react-gtm-module'

const ProductViewHeader = ({ name, price, currency, category, productType, rating, promotionPrice, promotions, description }) => {
    const intl = useIntl()
    ...
    return (
        <VStack mr={4} spacing={2} align="flex-start" marginBottom={[4, 4, 4, 0, 0]}>
            ...
            <Skeleton isLoaded={name}>
                <Heading fontSize="30">{`${name}`}</Heading> {/* Custom styling for PDP product name */}
            </Skeleton>
            {rating && (
                <RatingStar productRating={rating} />
            )}
            {/* Price */}
            <Skeleton isLoaded={price} minWidth={32}>

                {promotionPrice?.value < price ? (
                    <Flex justifyContent="flex-start" gap={2}>
                        {/* Custom Start: Display promotion price on PDP */}
                        <Text as="s" fontSize="20px" color="lightgray" aria-label="price">
                            {isProductASet &&
                                `${intl.formatMessage({
                        ...
                        {/* Custom End: Display promotion price on PDP */}
                        <Text fontWeight="bold" fontSize="20px" aria-label="price"> {/* Custom styling for product price on PDP */}
                            {isProductASet &&
                                `${intl.formatMessage({
                                    id: 'product_view.label.starting_at_price',
                                    defaultMessage: 'Starting at'
                                })} `}
                            {intl.formatNumber(promotionPrice?.value, {
                                style: 'currency',
                                currency: currency || activeCurrency
                            })}
                        </Text>
...
ProductViewHeader.propTypes = {
    ...
    promotionPrice: PropTypes.object,
            ...
            const handleCartItem = async () => {
                ...
                try {
                    ...
                    if (itemsAdded) {
                        ...
                        /**************** Custom start: Push add to cart detail into GTM dataLayer ****************/
                        if (product && itemsAdded.length > 0) {
                            const price = (itemsAdded[0].product?.c_promotionPrice?.value ?? itemsAdded[0].product.pricePerUnit) * itemsAdded[0].quantity
                                        ...
                                        products: [
                                            {
                                                ...
                                                quantity: itemsAdded[0].quantity
                                            }
                                        ]
                                    }
                                }
                    ...
                    >
                        {updateWishlist
                            ...
                            : isProductASet
                                ...
                                : buttonText.addToWishlist} <HeartIcon marginLeft={2} />
                    </ButtonWithRegistration>
                )
            }
            return buttons
            ...
            <Flex direction={'column'} data-testid="product-view" ref={ref}>
                ...
                <Box display={['block', 'block', 'block', 'none']}>
                    <ProductViewHeader
                        ...
                        currency={product?.currency}
                        category={category}
                        rating={product?.c_productRatings}
                        promotionPrice={product?.c_promotionPrice}
                        promotions={product?.c_promotions}
                    ...
                    <VStack align="stretch" spacing={8} flex={1}>
                        ...
                        <VStack align="stretch" spacing={4}>
                            ...
                            */}
                            ...
                            {/* Quantity Selector */}
                            {!isProductASet && (
                                <VStack align="stretch" maxWidth={'250px'}>
                                    {/* <Box fontWeight="bold">
                            ...
                            <Flex maxWidth="448px" justifyContent="space-between" mt="3px">
                                <Text fontWeight="700" fontSize="16px" lineHeight="32px">
                                    {intl.formatMessage({
                                        defaultMessage:
                                            'Sub Total'
                                    })}
                                ...
                                {/* Price */}
                                <Skeleton isLoaded={product?.price} minWidth={32} width="62px" height="32px">
                                    {product?.price > product?.c_promotionPrice?.value ? (
                                        <Text fontWeight="bold" fontSize="md" aria-label="price">
                                            ...
                                            {product && intl.formatNumber(product?.c_promotionPrice?.value, {
                                                style: 'currency',
                                                currency: product?.currency
                                            })}
                                        </Text>
                                        ...
                                        <Text fontWeight="bold" fontSize="md" aria-label="price">
                                            ...
                                            {product && intl.formatNumber(product?.price, {
                                                style: 'currency',
                                                currency: product?.currency
                                            })}
                                        </Text>
