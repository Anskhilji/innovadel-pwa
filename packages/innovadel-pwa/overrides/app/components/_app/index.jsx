...
import GoogleTagManager from '@innovadel/gtm/app/components/gtm-init'
// Localization
...
const App = (props) => {
    ...
    useEffect(() => {
        ...
        // Custom Start: Update customer email in basket
        if (baskets?.baskets?.[0] && baskets?.baskets?.[0].customerInfo?.email === "" && customer?.email) {
            updateCustomerForBasket.mutateAsync({
                parameters: {basketId: baskets.baskets[0].basketId },
                body: { email: customer.email }
            })
        }
        // Custom End: Update customer email in basket
    }, [baskets])
