...
import { Center } from '@salesforce/retail-react-app/app/components/shared/ui'
...
const ProductItem = ({
    ...
    // Custom Start: To get the current route
    const location = useLocation();
    const wishlistRoute = "/account/wishlist";
    // Custom End: To get the current route
    return (
        <Box position="relative" data-testid={`sf-cart-item-${product.productId}`}>
            <ItemVariantProvider variant={product}>
                ...
                <Stack layerStyle="cardBordered" align="flex-start">
                        ...
                        <Stack spacing={3} flex={1}>
                            ...
                            <Flex align="flex-end" justify="space-between">
                                ...
                                <Stack>
                                    ...
                                    {/* Custom Start: To display heart icon only on wishlist page */}
                                    <Box display={['none', 'flex', 'flex', 'flex']} alignItems={'Center'}>
                                        {location.pathname == wishlistRoute && (
                                            <Box justify-content="center" marginRight={'15px'} title={location.pathname}>
                                                <HeartIcon boxSize={8} color="red" />
                                            </Box>
                                        )}
                                        {primaryAction}
                                    </Box>
                                    {/* Custom End: To display heart icon only on wishlist page */}
                                </Stack>
                            </Flex>
                    ...
                    {/* Custom Start: To display heart icon only on wishlist page */}
                    <Box display={['flex', 'none', 'none', 'none']} w={'full'}>
                        {location.pathname == wishlistRoute && (
                            <Box justify-content="center" marginRight="15px" marginTop="5px">
                                {<HeartIcon boxSize={8} color="red" />}
                            </Box>
                        )}
                        {primaryAction}
