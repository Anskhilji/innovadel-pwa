...
// import {HeartSolidIcon} from '@salesforce/retail-react-app/app/components/icons'
...
const ProductTile = (props) => {
    ...
    return (
        ...
        >
            ...
            {/* Custom Start: custom styling product tiles */}
            <Box {...styles.tileWrapper}>
                {/* Title */}
                <Text {...styles.title}>{localizedProductName}</Text>
                {/* Price */}
                {product?.c_promotionPrice?.value < price ? (
                    <Flex justifyContent="flex-start" gap={2}>
                        <>
                            {/* Custom Start: Display promotion price on PLP */}
                            <Text as="s" {...styles.price} color="lightgray" data-testid="product-tile-price">
                                {hitType === 'set' &&
                                    intl.formatMessage({
                            ...
                            {/* Custom end */}
                            <Text {...styles.price} ml="0" data-testid="product-tile-price">
                                {hitType === 'set' &&
                                    intl.formatMessage({
                                        id: 'product_tile.label.starting_at_price',
                                        defaultMessage: 'Starting at'
                                    })}{' '}
                                {intl.formatNumber(product?.c_promotionPrice?.value, {
                                    style: 'currency',
                                    currency: currency || activeCurrency
                                })}
                            </Text>
                ...
                <Box {...styles.rating}>
                    <RatingStar productRating={props.product.c_rating}/>
                </Box>
            </Box>
            {/* Custom End */}
        </Link>
