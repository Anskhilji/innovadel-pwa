...
import EmptyCart from '../../../app/pages/cart/partials/empty-cart'
...
const Cart = () => {
    ...
    const changeItemQuantity = debounce(async (quantity, product) => {
        ...
        var updatedBasketResult = await updateItemInBasketMutation.mutateAsync(
            {
        ...
        if (updatedBasketResult) {
            const tagManagerArgs = {
                dataLayer: {
                    event: 'trackCart',
                    cart: {
                        cartItems: updatedBasketResult?.productItems.map((productItem) => {
                            return {
                                item: productItem.productId,
                                quantity: productItem.quantity,
                                price: productItem.priceAfterItemDiscount
                            }
                        })
                    }
                }
    ...
    const handleRemoveItem = async (product) => {
        setSelectedItem(product)
        setCartItemLoading(true)
        const removeItemFromBasketResult = await removeItemFromBasketMutation.mutateAsync(
            {
                parameters: { basketId: basket.basketId, itemId: product.itemId }
            },
            {
        ...
        if (removeItemFromBasketResult && product) {
            const price = product?.c_promotionPrice?.value ?? product.priceAfterItemDiscount
            const tagManagerArgs = {
                dataLayer: {
    ...
    /**************** Custom start: Full cart detail into GTM dataLayer ****************/
    useEffect(() => {
        if (basket?.productItems?.length) {
            const tagManagerArgs = {
                dataLayer: {
                    event: 'trackCart',
                    cart: {
                        cartItems: basket?.productItems.map((productItem) => {
                            return {
                                item: productItem.productId,
                                quantity: productItem.quantity,
                                price: productItem.priceAfterItemDiscount
