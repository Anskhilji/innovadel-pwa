...
import {useAccountPasswordChanged} from '../../hooks'
...
const ProfileCard = () => {
    ...
    return (
        ...
        >
            <ToggleCardEdit> 
                <Container variant="form" maxWidth='auto'>  {/* custom code for My Profile container */}
...
const handleAccountPasswordChanged = (accountPasswordChanged, customer) => {
    try {
        accountPasswordChanged.passwordChanged(customer)
    } catch (error) {
...
const PasswordCard = () => {
    ...
    const form = useForm()
    const accountPasswordChanged = useAccountPasswordChanged()
    const submit = async (values) => {
        try {
            form.clearErrors()
            updateCustomerPassword.mutate(
                ...
                },
                {
                    onSuccess: () => {
                        ...
                        //Custom start: calling custom endpoint for triggering marketing cloud on account password changed
                        handleAccountPasswordChanged(accountPasswordChanged, customer)
                        //Custom end
                    },
                    onError: () => {
                        form.setError('global', {type: 'manual', message: "Password is not updated please try again."})
                    }
                }
            )
        } catch (error) {
