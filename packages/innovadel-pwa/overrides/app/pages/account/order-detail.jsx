...
import OrderSummary from '../../../../app/components/order-tracking-summary'
...
const AccountOrderDetail = () => {
    ...
    const shippingAddress = order?.c_orderData?.OrderDeliveryGroupSummary
    const shippingMethod = order?.c_orderData?.OrderDeliveryGroupSummary?.OrderDeliveryMethod.Name
    const trackingNumber = order?.c_orderData?.OrderSummary?.PoNumber
    const shippingStatus = order?.c_orderData?.OrderSummary?.Status
    const paymentCard = order?.c_orderData?.paymentInstrument
    const CardIcon = getCreditCardIcon(paymentCard?.cardType)
    const itemCount = order?.c_orderData?.lineItems.reduce((count, item) => item.Quantity + count, 0) || 0

    const paymentInstrument = order?.paymentInstruments[0]
    const paymentMethodId = paymentInstrument && paymentInstrument.paymentMethodId
    const paypalToken = paymentMethodId === 'Paypal' ? JSON.parse(paymentCard.creditCardToken) : ''
    const paypalEmail = paymentMethodId === 'Paypal' && paypalToken ? paypalToken.userEmail : ''
                        ...
                        >
                            <Text fontSize={['sm', 'md']}>
                                <FormattedMessage
                                    ...
                                    values={{
                                        date: formatDate(
                                            new Date(order?.c_orderData?.OrderSummary?.CreatedDate),
                                            {
                                                year: 'numeric',
                                                day: 'numeric',
                                                month: 'short'
                            ...
                            <Text fontSize={['sm', 'md']}>
                                <FormattedMessage
                                    defaultMessage="Order Number4: {orderNumber}"
                                    id="account_order_detail.label.order_number"
                                    values={{orderNumber: order?.c_orderData?.OrderSummary?.OrderNumber}}
                                />
                            </Text>
                            <Badge colorScheme="green">{order?.c_orderData?.OrderSummary?.Status}</Badge>
                            ...
                            <>
                                ...
                                <Stack spacing={1}>
                                    ...
                                    <Box>
                                        ...
                                        <Text fontSize="sm">
                                            {order?.c_orderData?.OrderSummary?.BillingAddress.street}
                                        </Text>
                                        <Text fontSize="sm">
                                            {order?.c_orderData?.OrderSummary?.BillingAddress.city},{' '}
                                            {order?.c_orderData?.OrderSummary?.BillingAddress.state}{' '}
                                            {order?.c_orderData?.OrderSummary?.BillingAddress.postalCode}
                                        </Text>
                                    </Box>
                                </Stack>
                            </>
                ...
                <Stack spacing={4}>
                    {isLoading ? (
                        ...
                        <OrderProducts
                            ...
                            currency={order?.c_orderData?.currency}
                        />
                    )}
                </Stack>
            </Stack>
